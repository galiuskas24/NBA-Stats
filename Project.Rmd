...
plot(seasons$Year, seasons$X3P, type="l", col="green")

```

Također možemo zapaziti da današnji u prosjeku više zabiju trica neg tadašnji bek šuteri. To je također dokaz evolucije igre i uvježbavanja vještine kao što je šut za tri poena.
...
Ovo ostaljvam da VOCKO nesto pametno smislis
...
hist(sg$X3P, main = "Razdioba pogođenih trica SG", xlab = "Ukupan broj pogođenih trica po igraču - SG (1979-1984)", ylab = "Frekvencija")
summary(sg$X3P)
```
\bigbreak
```{r}
hist(centers$X3P, main = "Razdioba pogođenih trica C", xlab = "Ukupan broj pogođenih trica po igraču - C (2012-2017)", ylab = "Frekvencija")
...
```{r}
nan = qqPlot(standardize(wingspan$Wingspan.m), xlab = "Kvantili normalne distribucije", ylab = "",main = "Q-Q plot - Raspon ruku")
```
\bigbreak
```{r}
nan = qqPlot(standardize(wingspan$DWS), xlab = "Kvantili normalne distribucije", ylab = "",main = "Q-Q plot - DWS")
```
\bigbreak
```{r}
