...
# Setup (ovo treba pobrisati, ali sada je zgodno kliknuti desno i skočiti na setup u slučaju oslobađanja memorije)
...
# 2. TODO [Vlado] Evolucija trice
-prikaz trice po sezonama
-pokusat dokazat da danas centri sutaju više trica neg što su bek šuter šutali kad se tek uvela trica
...
# 4. TODO [Vlado] PLAYOFF vs REGULAR
```{r}
...
# 5. Utječe li širina raspona ruku na efikasnost u obrani? Kako bismo odgovorili na ovo pitanje moramo odrediti kako ćemo definirati efikasnost obrane. NBA statistika nam ovdje značajno pomaže sa podatkom DWS (Defensive Win Shares). To je složeni podatak koji uzima u obzir nekoliko ostalih podataka kao što su broj pogodaka, posjed lopte te neke timske podatke te na osnovu njih računa vlastitu vrijednost. Pokušat ćemo linearnom regresijom provjeriti jesu li ova dva podatka (wingspan i DWS) linearno povezani.
```{r}
#read.csv("nba-players-stats/wingspans.csv") -> wingspan
#wingspan %>% select(Player, WINGSPAN) %>% filter(WINGSPAN != "") -> player.wingspan
#player.wingspan$FEET = type.convert(player.wingspan$WINGSPAN, as.is = TRUE)
#player.wingspan$INCH = type.convert(player.wingspan$WINGSPAN, as.is = TRUE)
#player.wingspan$FEET = sapply(strsplit(player.wingspan$INCH, "\\s+"), '[', 1)
#player.wingspan$INCH = sapply(strsplit(player.wingspan$INCH, "\\s+"), '[', 2)
#player.wingspan$FEET = gsub("\'", "", player.wingspan$FEET)
#player.wingspan$INCH = gsub("\'", "", player.wingspan$INCH)
#player.wingspan$FEET = type.convert(player.wingspan$FEET, as.is = FALSE)
#player.wingspan$INCH = type.convert(player.wingspan$INCH, as.is = FALSE)
#player.wingspan$ftTom = conv_unit(player.wingspan$FEET, from = "ft", to = "m")
#player.wingspan$inchTom = conv_unit(player.wingspan$INCH, from = "inch", to = "m")
#player.wingspan$wing = player.wingspan$ftTom + player.wingspan$inchTom
#player.wingspan %>% select(Player, wing) %>%
#                    left_join(players.bySeasons) -> proba
#proba %>% select(Player, wing, DWS) %>%
#          na.omit() %>%
#          group_by(Player) %>%
#          summarise(averageDWS = sum(DWS) / n(), wingspan = sum(wing) / n()) %>%
#          filter(averageDWS > 0.5) -> novo

#novo
#qqPlot(standardize(novo$averageDWS), xlab = "Kvantili normalne distribucije", ylab = "")
#qqPlot(standardize(novo$wingspan), xlab = "Kvantili normalne distribucije", ylab = "")
#hist(novo$wingspan)
#hist(player.wingspan$wing)

#plot(novo$averageDWS~novo$wingspan, xlab = "prosječni wingspan", ylab = "prosječni defenzivni bodovi")
#fit = lm(novo$averageDWS~novo$wingspan)
#summary(fit)
...
qqPlot(standardize(wingspan$Wingspan.m), xlab = "Kvantili normalne distribucije", ylab = "")
qqPlot(standardize(wingspan$DWS), xlab = "Kvantili normalne distribucije", ylab = "")
hist(wingspan$Wingspan.m)
hist(wingspan$DWS)
plot(wingspan$DWS~wingspan$Wingspan.m, xlab = "prosječni wingspan", ylab = "prosječni DWS")
fit = lm(wingspan$DWS~wingspan$Wingspan.m)
summary(fit)

```
Na osnovu dobivenih rezultata zaključujemo da navedeni podatke ne možemo nikako povezati. Kada malo bolje razmislimo, veći raspon ruku omogućuje veću pokrivenost terena oko sebe, međutim, on također i unosi potencijalne negativne učinke kao što su tromost, manju ravnotežu i sl. Zbog toga ova statistika i nije toliko začuđujuća.



# 6. Hall of Fame igrači - najbolji baš u svemu?
...
Najprije ćemo vizualizirati podatke kako bi dobili ideju što možemo učiniti po pitanje provjere jesu li doista igrači bez završenog fakulteta bolji od onih koji su ga završili. Pritom ćemo zbog zgodnije vizualizacije igrače koji nisu završili fakultet označiti crvenom bojom, a one koji jesu zelenom.
```{r}
...
qqPlot(college.players$PTS, col = colors[1], main = "Poeni", xlab = "Kvantili normalne distribucije", ylab = "")
qqPlot(college.players$TRB, col = colors[1], main = "Skokovi", xlab = "Kvantili normalne distribucije", ylab = "")
qqPlot(non.college.players$PTS, col = colors[2], main = "Poeni", xlab = "Kvantili normalne distribucije", ylab = "")
qqPlot(non.college.players$TRB, col = colors[2], main = "Skokovi", xlab = "Kvantili normalne distribucije", ylab = "")
```
...
# 11. TODO [Vlado i Vocko] Povezanost duljine karijere i broja utakmica igrača.
Cilj ovog zadatka je napraviti dovoljno dobar model predikcije koji će na osnovu broja utakmica igrača predvidjeti koliko dugo je trajala karijera igrača.
```{r}
...
plot(all.players.train$Games~all.players.train$duration, ylab = "Broj utakmica igrača", xlab = "Trajanje karijere igrača (god.)")
fit = lm(all.players.train$duration ~ all.players.train$Games)
all.players.test$data = predict(fit, asd = all.players.test$Games)
summary(fit)
```
